...
def tester(*args):
    ...
    plot: False (sem gráfico), True (exibe) ou caminho de arquivo PNG (salva e exibe)
    '''
    ...
    if plot:
        _plot_performance(report, chart, lr.predict(X), line_f, line_b,
                          lr.score(X, y) * l, plot)

    return lr.score(X, y) * l


def _plot_performance(report, chart, trend, line_f, line_b, r2, plot):
    """Gráfico de desempenho da estratégia.

    - Azul: equity acumulada da estratégia (em pips)
    - Laranja: variação bruta do preço nas mesmas operações (benchmark —
      mostra que o lucro não vem só da direção do mercado)
    - Cinza tracejado: tendência linear (o R² do título mede a aderência a ela)

    `plot` pode ser True (só exibe) ou um caminho de arquivo (salva e exibe).
    """
    from viz_lib import (apply_style, save_fig, COLOR_STRATEGY, COLOR_BENCHMARK,
                         COLOR_TREND)
    apply_style()

    fig, ax = plt.subplots(figsize=(10, 5))
    ax.plot(report, color=COLOR_STRATEGY, linewidth=1.8,
            label='Equity da estratégia')
    ax.plot(chart, color=COLOR_BENCHMARK, linewidth=1.1, alpha=0.9,
            label='Variação do preço (benchmark)')
    ax.plot(trend, color=COLOR_TREND, linewidth=1.2, linestyle='--',
            label='Tendência linear')
    ax.set_title(f'Desempenho da estratégia — R² {r2:.2f}')
    ax.set_xlabel('Número de operações')
    ax.set_ylabel('Lucro acumulado (pips)')
