...
def export_model_to_ONNX(**kwargs):
    ...
    model[1].save_model(
        export_path + "catmodel_" + symbol + "_" + str(model_number) + ".onnx",
        format="onnx",
        export_parameters={
    ...
    model[2].save_model(
        export_path + "catmodel_m_" + symbol + "_" + str(model_number) + ".onnx",
        format="onnx",
        export_parameters={
            "onnx_domain": "ai.catboost",
            "onnx_model_version": 1,
    ...
    file = open(
        export_path + str(symbol) + "_ONNX_include_" + str(model_number) + ".mqh",
        "w",
    )
    file.write(code)
